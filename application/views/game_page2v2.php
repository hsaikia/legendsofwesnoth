...
<?php 
	...
	echo "<tr><td class='timestamp'>[". date('Y-m-d', strtotime($game->date)) ."]</td><td style='width:700'>" . getColoredHandle($game->handle_w1, $game->rating_w1) . " (" . $faction[$game->faction_w1] . ") and " . getColoredHandle($game->handle_w2, $game->rating_w2) . " (" . $faction[$game->faction_w2] . ") ";
	echo " defeated ";
...
foreach ($comments as $comment) {
	echo "<tr><td class='timestamp'>[". date('Y-m-d', strtotime($comment->date)) ."]</td><td style='width:100px;'><b>". getColoredHandle($comment->handle, $comment->rating) . "</b> :</td><td>" . $comment->text ."</td></tr>";
} 
